...
export default function DCL({ loan, onBack }) {
  ...
  // Only after form submit do we mark the document as deferralRequested
  ...
  const displayedDocuments = isPending
    ? documents
    : documents.filter(d => d.status !== "Pending" && d.status !== "Pending from RM" && d.status !== "Pending from CO");
  ...
  // If a deferral is active, render only the form
  ...
  // Otherwise render main DCL table + comment
  return (
